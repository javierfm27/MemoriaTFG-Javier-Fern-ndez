...
\begin{figure}[ht]
    ...
    \caption{ Esquema de los estados de un repositorio Git \\ Fuente: \url{https://git-scm.com/book/es/v1/Empezando-Fundamentos-de-Git}}
    \label{fig:gitStates}
\end{figure}
\begin{itemize}
...
\begin{figure}[H]
    ...
    \caption{WorkFlow impuesto por la estrategia y mantenimiento de \emph{GitFlow} \\ Fuente: \url{https://datasift.github.io/gitflow/IntroducingGitFlow.html}}
    \label{fig:gitFlow}
\end{figure}
Por tanto, cada vez que se incorpora código a la rama \textit{master}, tenemos nueva versión. Además de las ramas principales, observando la figura \ref{fig:gitFlow} podemos distinguir tres tipos de ramas auxiliares, donde cada una sigue sus propias reglas:
\begin{itemize}
...
\end{itemize}
Así, esta herramienta nos permitirá corregir bugs detectados por usuarios de \emph{PyCardio}, donde estos nos informaran mediante la apertura de \emph{tickets}.
...
Antes de introducir Anaconda debemos saber que es Conda. Conda es un gestor de paquetes y de entornos virtuales que nos permite instalar, ejecutar y actualizar los paquetes que nosotros deseemos. Como entorno virtual se conoce al conjunto de ejecutables, en este caso, de Python y las distintas versiones de librerías que queremos usar permitiéndonos desarrollar varias aplicaciones con distintas versiones tanto de librerías como del propio intérprete del lenguaje, es decir, Python. \\
 Debido al carácter científico de nuestro módulo y por la versatilidad de Anaconda, realizaremos un empaquetado para su distribución en Conda. ¿Por qué este interés en distribuir nuestro módulo en Conda? Aparte de lo mencionado, Conda goza de las siguientes características que la convierten en una de las mejores herramientas para ciencia de datos con Python:
 \begin{itemize}
 ...
 Para generar el empaquetado de conda. Se han usado los módulos \textit{conda skeleton} y \textit{conda build}. \\
 ...
 \begin{itemize}
     ...
     \item \textbf{build.sh: } Script que instala todos los archivos para el paquete en Linux y macOs.
     \item \textbf{bld.bat: } Misma función que \textit{build.sh} pero para la plataforma Windows.
     \item \textbf{$run\_test.[py,pl,sh,bat]$:} Fichero opcional de testeo. Script que contiene los comandos de ejecución para el testeo del módulo o software si estos son necesarios para la receta.
 \end{itemize}
 
 Al contar con el empaquetado en PyPI, usaremos la función que ofrece \emph{conda skeleton} de construir nuestra receta a partir del contenido que se aloja en PyPI.
...
\begin{enumerate}
    ...
    \item Cómo versionar la documentación.
\end{enumerate}
...
\begin{figure}[H]
    ...
    \caption{Ejemplo de documentación generada por \emph{Read the Docs} \\Fuente: \url{https://sphinx-autoapi.readthedocs.io/en/latest/}}
...
Una vez aclarado, la ventaja principal, aparte de todas las proporcionadas por ser un repositorio \emph{Git}, es que al no necesitar base de datos para servir el contenido, solo se requiere un servidor web que vaya sirviendo el contenido que se solicita.
...
Jekyll está escrito en Ruby, donde las gemas (\emph{gems}) serían las librerías que se desarrollan y se usan para cualquier proyecto escrito en este lenguaje. Un proyecto como \emph{Jekyll}, puede usar un gran abanico de librerías por lo que instalar una a una individualmente puede resultar un trabajo bastante engorroso. Para ello, se utiliza una librería (gema) que se encargará de manejar todas estas dependencias. Esta gema se denomina, \emph{Bundler}. Esta herramienta o librería instalará todas aquellas dependencias que vengan contenidas en el archivo \texttt{gemfile}.  \\
\begin{figure}[H]
    \centering
...
\begin{lstlisting}[style=htmlcssjs,caption=default.html]
<!-- default.html -->
<footer>
    {{include footer.html}}
</footer>
...
Hay que destacar la herramienta Bundler que introduciamos en \ref{subsec:jekyll}, ya que nos ayuda a mantener las dependencias actualizadas. La versión de la web subida en GitHub puede estar desfasada con la que tenemos desarrollada en local.
